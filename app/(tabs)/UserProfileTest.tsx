...
const UserProfileTest = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const data = isAdmin ? await getAdminById(id) : await getTruckerById(id);
        setUserData(data);
      } catch (err) {
  ...
  if (loading) {
    return <View style={styles.loader}><ActivityIndicator size="large" color="#088395" /></View>;
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.profileCard}>
        <View style={styles.imageWrapper}>
          <Image
            source={userData?.profile_pic_url ? { uri: userData.profile_pic_url } : require('../../assets/images/prof.png')}
...
const styles = StyleSheet.create({
  ...
  profileCard: { alignItems: 'center', padding: 24, backgroundColor: '#FFFFFF', margin: 16, borderRadius: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 6, elevation: 3 },
  imageWrapper: { position: 'relative' },
  profileImage: { width: 120, height: 120, borderRadius: 60, borderColor: '#E2E8F0', borderWidth: 2 },
  imageOverlayButton: { position: 'absolute', bottom: 0, right: 0, backgroundColor: '#088395', width: 32, height: 32, borderRadius: 16, justifyContent: 'center', alignItems: 'center' },
  ...
  infoCard: { backgroundColor: '#FFFFFF', marginHorizontal: 16, padding: 20, borderRadius: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2, marginTop: 16 },
